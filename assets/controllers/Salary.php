		...
		if (isset($teacher_id)) {
			...
			foreach ($data['classes_data'] as $key => $value) {
				...
				if (isset($salaries[$value->student_id]['work_mins_15'])) {
					...
					$hour_15 = $salaries[$value->student_id]['work_mins_15'] / 60;

					$salaries[$value->student_id]['work_hours_15'] = number_format($hour_15,2);
				}

				if (isset($salaries[$value->student_id]['work_mins_30'])) {
					$hour_30 = $salaries[$value->student_id]['work_mins_30'] / 60;

					$salaries[$value->student_id]['work_hours_30'] = number_format($hour_30,2);
				}


				$salaries[$value->student_id]['salary_15'] = $teacher_rate*$salaries[$value->student_id]['work_hours_15'];
				$salaries[$value->student_id]['salary_30'] = $teacher_rate*$salaries[$value->student_id]['work_hours_30'];
