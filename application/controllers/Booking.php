...
class Booking extends Ice_Controller {
	...
	public function add_booking(){
		...
		if (!$is_user_logged_in) {
			...
			$response['error'] = $this->lang->line('you_must_login');
			...
			if ($action == "regular_class") {
				...
				$ticket = $this->crud_model->get_data("tickets",array("student_id"=>$user_id,"end_date >"=>$current_date),true,array("pricing_plans"=>"pricing_plans.plan_id=tickets.plan_id"));
				...
				if (empty($ticket)) {
					...
					if ($current_date > $ticket->end_date || $ticket->remaining_classes <= 0 ) {
						...
						$tickets_where['student_id'] = $user_id;
						$tik = $this->crud_model->get_data("tickets",$tickets_where,true);

						$plan_where['plan_id'] = $tik->plan_id;
						$plan = $this->crud_model->get_data("pricing_plans",$plan_where,true);

						 $classes_in_month = $plan->total_classes;
					...
					if (!isset($response['error'])) {
						...
						$prev_classes = $this->crud_model->get_data("classes",array("student_id"=>$user_id),'',array("availability"=>"availability.id=classes.availability_id"));
				...
				if (!isset($response['error'])) {
					...
					if(sizeof($events) > $tik->remaining_classes && $tik->extra_classes == 0)
					{
						$response['error'] = $this->lang->line('you_are_not_allowed_to_take_more_than')." {$classes_in_month} ".$this->lang->line('classes_in_a_month');
					}
					else
				...
				$level_test = $this->crud_model->get_data("classes",array("student_id"=>$user_id,"ticket_id"=>"level_test"),true);
	...
	public function change_class_status(){
		...
		if(empty($check))
		{
			$memo = $this->crud_model->add_data("memos",array("ticket_id"=>$ticket_id,'memo'=>$memo));
		}
		else
		{
			$memo = $this->crud_model->update_data("memos",array("ticket_id"=>$ticket_id),array("memo"=>$memo));
		}
	...
	public function get_booking(){
		...
		if ($is_user_logged_in) {
	        ...
	        $join['availability a'] = 'a.id=c.availability_id';
	        $where['a.start'] = $start;
	        $where['a.end'] = $end;
	        $where['c.student_id'] = get_user_id();
	    
	        $booking = $this->crud_model->get_data('classes c',$where,true,$join);
